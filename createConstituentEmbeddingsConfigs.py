        ...
        for aggSent in ["0", "1", "2", "3"]:
            for entity in ["entity", "keywordEntity"]:
                for label in ["entityPrefLabel", "entityLabel", "matchingEntityLabel", "entityDescription"]:
                    f.write('\t<bean id="constituentEmbeddingsFeature' + aggKey + aggSent + entity[0] + getAbbreviation(label) + '" class="edu.kit.anthropomatik.isl.newsTeller.retrieval.ranking.features.ConstituentEmbeddingsFeature">\n')
                    ...
                    f.write('\t\t<property name="name" value="constituentEmbeddings' + aggKey + aggSent + entity[0] + getAbbreviation(label) + '" />\n')
